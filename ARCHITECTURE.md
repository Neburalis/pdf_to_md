...
**Решение:** CLI разделён на `convert`, `detect`, `clear-cache` и `help`. Аргументы и валидация прежнего основного flow перенесены в `convert`; quality tools перенесены из вспомогательных скриптов в пакетные модули `hallucinations.py` и `cache_quality.py`, а `detect --no-gemma` позволяет запускать только быстрые проверки.

**Результат:** конвертация и служебные операции доступны через единую точку входа `python -m pdf_to_md`; `help` не загружает pipeline или модели, а собранный wheel содержит всю логику quality tools.
